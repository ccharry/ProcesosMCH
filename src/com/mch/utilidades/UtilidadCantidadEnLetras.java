...
public class UtilidadCantidadEnLetras {

  public UtilidadCantidadEnLetras() {
  }
  ...
  private static String[] xDos  = new String[]{"U|NO","UN","DOS","TRES","CUA|TRO","CIN|CO",
                             ...
                             "DIE|CI|NUE|VE",
                             "VEIN|TE","VEIN|TI|UN","VEIN|TI|DOS","VEIN|TI|TRES",
  ...
  private static int[]  renglones = new int[] { 2 };
  ...
  private static void traducir(int mil)    {
      ...
      //Add Jonnatan 09/05/2011 
      if(paso == 2 && unidad == 1 && decena == 0 && (uno > 0 || posicion > 0) && dos != 21){
    	  pasarTexto(xDos[0]);
      }      
      //Fin Jonnatan
      ...
      //Add Jonnatan 09/05/2011 
      if ( paso == 3 && unidad == 1 && decena == 0 && (uno > 0 || posicion > 0) && dos != 21){
    	  pasarTexto(xDos[0]);
    	  }
      //Fin Jonnatan
...
//Valor por defecto "" . O sea que si la parte entera == 0 no se incluye nada
      UtilidadCantidadEnLetras.setPalabra_cero("");
...
//se pasa a false
      UtilidadCantidadEnLetras.setCantidad_decimales(1);
...
//valor 0 , traduce a "UNO" (valor por defecto) , valor 1 traduce a "UNA" y valor 2 a "UN"
      UtilidadCantidadEnLetras.setGenero_unidad(2);
...
//seria mas logico " CEN|TA|VOS.-"
      UtilidadCantidadEnLetras.setSufijo_decimales("");
...
//COMPLETA AL RENGLON SIGUIENTE
      UtilidadCantidadEnLetras.setSufijo_enteros("");
//Fija el texto a agregar al final
      UtilidadCantidadEnLetras.setSufijo_final("");
...
//haya que seguir en el proximo se completaran con el caracter "-"
      UtilidadCantidadEnLetras.setCaracter_proteccion('*');
...
//usando el metodo que recibe un double cuando la cantidad es muy grande
      String[] texto = UtilidadCantidadEnLetras.getTexto(valor);
...
*/        
      ...
      for ( int i = 0 ; i < texto.length; i++ )   {
        imprime = imprime + texto[i];
      }
      
      imprime = imprime.substring(0,imprime.indexOf("*"));
      if (imprime.endsWith ("MILLONES")||imprime.endsWith ("MILLON"))
        imprime = imprime +" DE";        
...
/*        RMCantidadEnLetras.setPrefijo_inicio("(");
      ...
      RMCantidadEnLetras.setSufijo_decimales("MI|LI|GRA|MOS.-");
      RMCantidadEnLetras.setSufijo_enteros("KI|LO|GRA|MOS");
      texto = RMCantidadEnLetras.getTexto(cantidad);
      for ( int i = 0 ; i < texto.length; i++ )   {
          System.out.println(texto[i]);
      } 
...
//    String importe = "1101.0";
//    int codigo;
//    importe = UtilidadCantidadEnLetras.imprimir(importe); 
//    System.out.println(importe);
	  System.out.println(UtilidadCantidadEnLetras.imprimir("100.0"));;
  }
}
